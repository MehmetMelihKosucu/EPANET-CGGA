...
//
// The Pipe objects are deliberately not destroyed: a Pipe normally belongs to
// a Network, and destroying one outside that context is not part of what these
// tests are intended to exercise.
// ---------------------------------------------------------------------------
void testRegistrationAndClear()
{
    ...
    Pipe* pipe = new Pipe("TEST_PIPE_1");
    ...
    Pipe* other = new Pipe("TEST_PIPE_2");
...
//
// A query for a pipe the manager does not know about must fail cleanly. This
// is the property the subsystem must not violate: an unsatisfiable reach-back
// has to report found == false, because a silently extrapolated value would
// enter the characteristic equation indistinguishably from a committed one.
// ---------------------------------------------------------------------------
void testReachBackGuard()
{
    std::printf("FlowHistoryManager reach-back guard\n"); std::fflush(stdout);

    FlowHistoryManager& m = FlowHistoryManager::getInstance();
    m.clear();

    Pipe* pipe = new Pipe("TEST_PIPE_3");
