...
class Pattern: public Element
{
  public:
    ...
    enum InterpolationType { DISCRETE, LINEAR };  // NEW: Interpolation modes
    ...
    virtual void   init(double intrvl, double tstart) = 0;
    virtual double    nextTime(double t) = 0;
    virtual void   advance(double t) = 0;

    // NEW: Interpolation settings
    InterpolationType interpType;      // DISCRETE or LINEAR
    double currentTime;                // Current time for interpolation

    double factorAt(double t);                     // NEW: Returns interpolated factor at time t
    void   setInterpolation(InterpolationType it) { interpType = it; }

    virtual double interpolatedFactor(double t) = 0;
  ...
  protected:
    ...
    double                 interval;       //!< fixed time interval (sec)
};
...
class FixedPattern : public Pattern
{
  public:
    ...
    // Methods
    void   init(double intrvl, double tstart) override;
    double nextTime(double t) override;
    void   advance(double t) override;
    double interpolatedFactor(double t) override;  // NEW

  private:
    double    startTime;   //!< offset from time 0 when the pattern begins (sec)
};
...
class VariablePattern : public Pattern
{
  public:
    ...
    double time(int i) { return times[i]; }
  ...
  private:
    std::vector<double>   times;  //!< times (sec) at which factors change
};
