...
Pump::Pump(const Pump& other) : Link(other)
{
	speed = other.speed;
	speedPattern = other.speedPattern;
	efficCurve = other.efficCurve;
	costPattern = other.costPattern;
	costPerKwh = other.costPerKwh;
}

Pump* Pump::clone() const
{
	Pump* newPump = new Pump(*this);
	newPump->speedPattern = speedPattern;
	newPump->efficCurve = efficCurve;
	newPump->costPattern = costPattern;
	newPump->costPerKwh = costPerKwh;
	newPump->pumpEnergy = pumpEnergy;
	newPump->pumpCurve = pumpCurve;
	newPump->initStatus = initStatus;
	newPump->initSetting = initSetting;
	return newPump;
...
//-----------------------------------------------------------------------------
//// ????????????????????????????????????????????????????????????????????????????????????????????????????????
void Pump::applyControlPattern(double currentTime, ostream& msgLog)
{
    if (speedPattern)
    {
        // Use interpolated factor for smooth pump speed changes
        double speedFactor = speedPattern->factorAt(currentTime);
        speedFactor = std::max(0.0, speedFactor);  // Speed can't be negative
