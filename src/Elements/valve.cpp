...
Valve::Valve(const Valve& other) : Link(other)
{
	valveType = other.valveType;
	presManagType = other.presManagType;
	lossFactor = other.lossFactor;
	setting = other.setting;
	status = other.status;
	initStatus = other.initStatus;
	initSetting = other.initSetting;
	hasFixedStatus = other.hasFixedStatus;
	elev = other.elev;
	settingPattern = other.settingPattern;
	pastFlow = other.pastFlow;
	pastHloss = other.pastHloss;
	pastSetting = other.pastSetting;
	remoteNode = other.remoteNode;
}

Valve* Valve::clone() const
{
    // where are valve attributes?
	Valve* newValve = new Valve(*this);
	newValve->remoteNode = remoteNode;
	newValve->settingPattern = settingPattern;
	newValve->initStatus = VALVE_ACTIVE;
	newValve->initSetting = 0;
	newValve->status = Link::LINK_CLOSED;
	newValve->setting = 0;
	newValve->pastFlow = 0;
	newValve->pastHloss = 0;
	newValve->pastSetting = 0;
	newValve->hasFixedStatus = false;
	newValve->elev = 0.0;
	newValve->lossFactor = 0.0;
	newValve->valveType = valveType;
    newValve->presManagType = presManagType;
	return newValve;
...
void Valve::applyControlPattern(double currentTime, ostream& msgLog)
{
    if (settingPattern)
    {
        // Use interpolated factor for smooth valve operation
        // This is essential for water hammer simulations
        double fs = settingPattern->factorAt(currentTime);

        // Clamp to valid range [0, 1] for safety
        fs = std::max(0.0, std::min(fs, 1.0));

        // Optional: Log significant changes for debugging
        /*
        if (std::abs(fs - setting) > 0.001) {
            msgLog << "\n  Valve " << name() << " setting: "
                   << setting << " -> " << fs
                   << " at t=" << currentTime << "s";
        }
