...
bool Network::isDemandModelPressureDependent() const
{
    return demandModel && !dynamic_cast<FixedDemandModel*>(demandModel);
}
...
// Add the findLink method
Link* Network::findLink(const std::string& name) {
    auto it = linkTable.find(name);
    return it != linkTable.end() ? static_cast<Link*>(it->second) : nullptr;
}
