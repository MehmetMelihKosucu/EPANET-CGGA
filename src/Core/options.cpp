...
static const char* hydSolverWords[] = { "GGA", "RWCGGA", "CGGA", "GAWH", 0};
...
/*SolverMode Options::getHydSolverMode() {
    // Debugging: Print all entries in stringOptions
    for (const auto& pair : stringOptions) {
        std::cout << pair.first << " = " << pair.second << std::endl;
    }

    // Ensure HYD_SOLVER exists
    auto it = stringOptions.find("HYD_SOLVER");
    if (it == stringOptions.end()) {
        throw std::runtime_error("HYD_SOLVER option is not set.");
    }

    const std::string solver = it->second;

    if (solver == "GGA") return QUASI_STEADY;
    if (solver == "RWCGGA") return RWC;
    if (solver == "CGGA") return WATER_HAMMER;

    return QUASI_STEADY;  // Default fallback
} // */
