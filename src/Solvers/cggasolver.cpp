...
int CGGASolver::solve(double& tstep_, int& trials, double currentTime) {
    ...
    if ((simulationPhase == FRONT_END || simulationPhase == BACK_END) && newEventDetected) {
        ...
        if (aboveDynamic) {
            ...
            // Realign currentSecondState with the COMMITTED WH state so the next
            // integer second differences against WH(t), not the discarded probe.
            // (Do not call computeFlowIndicators again — that would re-shift
            //  prevSecondState onto the probe.)
            for (Node* node : network->nodes)
            ...
            tstep_ = tstep;              // continue WH at Delta_WH

            network->msgLog << "\n  Indicators above dynamic -> WATER HAMMER engaged at t="
                << currentTime << " (phiA=" << maxPhiA << ", phiR=" << maxPhiR << ")";
            return result;
        }
        ...
        if ( !node->fixedGrade )
        {
            ...
            if ( node->type() == Node::TANK && theta != 0.0 )
            {
				...
				else
				{
                    ...
                    double al = a * tank->pastHead + (1.0 - theta) * tank->pastOutflow / theta;
