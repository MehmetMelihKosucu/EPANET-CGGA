...
public:
    ...
    double phiA_D = 0.1; // 0.5; // 0.1; // 0.5;    // Dynamic threshold [m]
    double phiR_D = 0.08; // 0.02; // 0.1;   // Dynamic threshold [-]
    double phiA_I = 0.05; // 0.25; // 0.05; // 0.2;  // 0.001; // 0.1;    // Inertial threshold [m]
    double phiR_I = 0.04; // // 0.01; // 0.04; // 0.0005; // 0.05;   // Inertial threshold [-]
    ...
    // define time steps:
    static constexpr double Delta_WH = 0.01; // Water Hammer step
...
private:
    ...
    std::vector<double> redoNodeHeads;       // Pre-solve node heads
    std::vector<double> redoLinkFlows;       // Pre-solve link flows
    std::vector<double> redoPipeStartFlows;  // Pre-solve pipe start flows
    std::vector<double> redoPipeEndFlows;    // Pre-solve pipe end flows

    int CGGASolver::sign(double val);

    bool pendingModeChange = false;

    bool whToRwcTransition = false;
